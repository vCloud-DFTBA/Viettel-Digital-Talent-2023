db = db.getSiblingDB("list_candidate_VDT");
db.article.drop();

db.candidate.save( {
    "STT": 1,
   "fullname": "Bùi Minh Sơn",
   "year of birth": 2002,
   "gender": "Nam",
   "university": "Đại học Công nghệ - Đại học Quốc gia Hà Nội",
   "Username": "sonbm",
   "field": "Công nghệ thông tin"
});

db.candidate.save( {
    "STT": 2,
   "fullname": "Đào Đại Hiệp",
   "year of birth": 2001,
   "gender": "Nam",
   "university": "Đại học Bách khoa Hà Nội",
   "Username": "hiepdd",
   "field": "Điện tử viễn thông"
});

db.candidate.save( {
    "STT": 3,
   "fullname": "Đỗ Anh Tú",
   "year of birth": 2002,
   "gender": "Nam",
   "university": "Đại học Công nghệ - Đại học Quốc gia Hà Nội",
   "Username": "tuda",
   "field": "Mạng máy tính và truyền thông dữ liệu"
});

db.candidate.save( {
    "STT": 4,
   "fullname": "Đỗ Bảo Hoàng",
   "year of birth": 1997,
   "gender": "Nam",
   "university": "Đại học CNTT, cơ khí &amp; quang học St.Petersburg LB Nga",
   "Username": "hoangdb",
   "field": "An toàn thông tin"
});

db.candidate.save( {
    "STT": 5,
   "fullname": "Hoàng Quốc Doanh",
   "year of birth": 2001,
   "gender": "Nam",
   "university": "Đại Học Bách Khoa Hà Nội",
   "Username": "doanhhq",
   "field": "NULL"
});

db.candidate.save( {
    "STT": 6,
    "fullname": "Le Minh Duc",
    "year of birth": 2002,
    "gender": "Nam",
    "university": "Đại Học Bách Khoa Hà Nội",
    "Username": "duclm",
    "field": "Công nghệ thông tin ứng dụng phần mềm"
});


db.candidate.save( {
    "STT": 7,
    "fullname": "Lê Phúc Lai",
    "year of birth": 2002,
    "gender": "Nam",
    "university": "Đại Học Bách Khoa Hà Nội",
    "Username": "lailp",
    "field": "Kỹ thuật điện tử viễn thông"
});


db.candidate.save( {
    "STT": 8,
    "fullname": "Lê Quang Anh",
    "year of birth": 1997,
    "gender": "Nam",
    "university": "Đại học CNTT, cơ khí &amp; quang học St.Petersburg LB Nga",
    "Username": "anhlq",
    "field": "An toàn thông tin"
});

db.candidate.save( {
    "STT": 9,
    "fullname": "Lê Trọng Minh",
    "year of birth": 2000,
    "gender": "Nam",
    "university": "Đại học CNTT, cơ khí &amp; quang học St.Petersburg LB Nga",
    "Username": "minhlt",
    "field": "Kỹ thuật điều khiển và tự động hóa"
});

db.candidate.save( {
    "STT": 10,
    "fullname": "Lê Tùng Lâm",
    "year of birth": 1999,
    "gender": "Nam",
    "university": "Đại Học Bách Khoa Hà Nội",
    "Username": "lamlt",
    "field": "Khoa học máy tính"
});

db.candidate.save( {
    "STT": 11,
    "fullname": "Lê Văn Chiến",
    "year of birth": 2002,
    "gender": "Nam",
    "university": "Đại học Công nghệ - Đại học Quốc gia Hà Nội",
    "Username": "chienlv",
    "field": "Kỹ thuật hàng không vũ trụ"
});

db.candidate.save( {
    "STT": 12,
   "fullname": "Linh Thi Nguyen",
   "year of birth": 2002,
   "gender": "Nữ",
   "university": "Đại Học Bách Khoa Hà Nội",
   "Username": "linhnt",
   "field": "ICT"
});


db.candidate.save( {
    "STT": 13,
    "fullname": "Nguyễn Đại An",
    "year of birth": 2023,
    "gender": "Nam",
    "university": "Đại Học Bách Khoa Hà Nội",
    "Username": "annd",
    "field": "Khoa học máy tính"
});

db.candidate.save( {
    "STT": 14,
    "fullname": "Nguyễn Đình Hoàng",
    "year of birth": 2002,
    "gender": "Nam",
    "university": "Đại học Công nghệ - Đại học Quốc gia Hà Nội",
    "Username": "hoangnd",
    "field": "Công nghệ thông tin ứng dụng phần mềm"
});

db.candidate.save( {
    "STT": 15,
   "fullname": "Nguyen Duc Vinh Data",
   "year of birth": 2002,
   "gender": "Nam",
   "university": "Học viện Công nghệ Bưu chính Viễn thông",
   "Username": "vinhnd",
   "field": "NULL"
});

db.candidate.save( {
    "STT": 16,
    "fullname": "Nguyễn Dương Long",
    "year of birth": 2002,
    "gender": "Nam",
    "university": "ĐH Thuỷ lợi",
    "Username": "longnd",
    "field": "Công nghệ thông tin ứng dụng phần mềm"
});

db.candidate.save( {
    "STT": 17,
    "fullname": "Nguyen Huu Thang",
    "year of birth": 2000,
    "gender": "Nam",
    "university": "Đại Học Bách Khoa Hà Nội",
    "Username": "thangnh",
    "field": "Khoa học máy tính"
});



db.candidate.save( {
    "STT": 18,
   "fullname": "Nguyễn Mạnh Cường",
   "year of birth": 1997,
   "gender": "Nam",
   "university": "Đại học CNTT, cơ khí &amp; quang học St.Petersburg LB Nga",
   "Username": "cuongnm",
   "field": "Điện tử"
});

db.candidate.save( {
    "STT": 19,
    "fullname": "Nguyễn Mạnh Đức",
    "year of birth": 2002,
    "gender": "Nam",
    "university": "Học viện Kỹ thuật mật mã",
    "Username": "ducnm",
    "field": "An toàn thông tin"
});

db.candidate.save( {
    "STT": 20,
   "fullname": "Nguyễn Ngọc Chung",
   "year of birth": 2002,
   "gender": "Nam",
   "university": "Học viên Công nghệ Bưu chính Viễn Thông HCM",
   "Username": "chungnn",
   "field": "NULL"
});

db.candidate.save( {
    "STT": 21,
    "fullname": "Nguyễn Tuấn Anh",
    "year of birth": 1997,
    "gender": "Nam",
    "university": "Đại học CNTT, cơ khí &amp; quang học St.Petersburg LB Nga",
    "Username": "anhnt",
    "field": "An toàn thông tin"
});

db.candidate.save( {
    "STT": 22,
   "fullname": "Nguyễn Văn Quang",
   "year of birth": 1997,
   "gender": "Nam",
   "university": "Đại học CNTT, cơ khí &amp; quang học St.Petersburg LB Nga",
   "Username": "quangnv",
   "field": "An toàn thông tin"
});

db.candidate.save( {
    "STT": 23,
   "fullname": "Ninh Chí Hướng",
   "year of birth": 2002,
   "gender": "Nam",
   "university": "Học viện Công nghệ Bưu chính viễn thông",
   "Username": "huongnc",
   "field": "An toàn thông tin"
});

db.candidate.save( {
    "STT": 24,
    "fullname": "Ninh Văn Nghĩa",
    "year of birth": 2001,
    "gender": "Nam",
    "university": "Đại Học Bách Khoa Hà Nội",
    "Username": "nghianv",
    "field": "Khoa học máy tính"
});

db.candidate.save( {
    "STT": 25,
    "fullname": "Phạm Anh Đức",
    "year of birth": 2001,
    "gender": "Nam",
    "university": "Đại học Bách Khoa Hà Nội",
    "Username": "ducpa",
    "field": "Toán ứng dụng và tin học"
});

db.candidate.save( {
    "STT": 26,
    "fullname": "Phạm Duy Cương",
    "year of birth": 1997,
    "gender": "Nam",
    "university": "Đại học CNTT, cơ khí &amp; quang học St.Petersburg LB Nga",
    "Username": "cuongpd",
    "field": "Công nghệ điện tử"
});

db.candidate.save( {
    "STT": 27,
    "fullname": "Phạm Hồng Thanh",
    "year of birth": 1998,
    "gender": "Nam",
    "university": "Swinburne University",
    "Username": "thanhph",
    "field": "Phát triển phần mềm"
});

db.candidate.save( {
    "STT": 28,
    "fullname": "Phạm Thị Khánh Linh",
    "year of birth": 2002,
    "gender": "Nữ",
    "university": "Đại học Công nghệ - Đại học Quốc gia Hà Nội",
    "Username": "linhptk",
    "field": "Mạng máy tính và truyền thông dữ liệu"
});

db.candidate.save( {
    "STT": 29,
   "fullname": "Phạm Văn Tới",
   "year of birth": 2002,
   "gender": "Nam",
   "university": "Học viện Công nghệ Bưu chính viễn thông",
   "Username": "toipv",
   "field": "Công nghệ thông tin ứng dụng phần mềm"
});

db.candidate.save( {
    "STT": 30,
    "fullname": "Trần Đức Mạnh",
    "year of birth": 1998,
    "gender": "Nam",
    "university": "Đại học CNTT, cơ khí &amp; quang học St.Petersburg LB Nga",
    "Username": "manhtd",
    "field": "Bảo mật thông tin"
});

db.candidate.save( {
    "STT": 31,
   "fullname": "Trần Mạnh Dũng",
   "year of birth": 2001,
   "gender": "Nam",
   "university": "Học viện Công nghệ Bưu chính Viễn thông",
   "Username": "dungtm",
   "field": "Điện tử viễn thông"
});

db.candidate.save( {
    "STT": 32,
    "fullname": "Trần Minh Dương",
    "year of birth": 2002,
    "gender": "Nam",
    "university": "Đại học Công nghệ - Đại học Quốc gia Hà Nội",
    "Username": "duongtm",
    "field": "Mạng máy tính và truyền thông dữ liệu"
});

db.candidate.save( {
    "STT": 33,
    "fullname": "Trần Xuân Phú",
    "year of birth": 2001,
    "gender": "Nam",
    "university": "Trường Đại học Công nghệ thông tin - ĐHQG Tp.Hồ chí Minh",
    "Username": "phutx",
    "field": "Khoa học máy tính"
});


db.candidate.save( {
    "STT": 34,
    "fullname": "Vũ Hoàng Long",
    "year of birth": 2001,
    "gender": "Nam",
    "university": "Đại học Bách Khoa Hà Nội",
    "Username": "longvh",
    "field": "Khoa học máy tính"
});


db.candidate.save( {
    "STT": 35,
    "fullname": "Vũ Minh Hiếu",
    "year of birth": 2000,
    "gender": "Nam",
    "university": "Đại học CNTT, cơ khí &amp; quang học St.Petersburg LB Nga",
    "Username": "hieuvm",
    "field": "Kỹ thuật phần mềm"
});

